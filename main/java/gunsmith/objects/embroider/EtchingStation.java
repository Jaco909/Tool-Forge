package gunsmith.objects.embroider;
...
public class EtchingStation extends CraftingStationObject {
    public ObjectDamagedTextureArray texture;

    public EtchingStation() {
    ...
    public void interact(Level level, int x, int y, PlayerMob player) {
        if (level.isServer()) {
            GunsmithContainer.openAndSendContainer(ContainerRegistry.RENAME_ITEM_CONTAINER, player.getServerClient(), level, x, y);
        }
    }

    public ObjectEntity getNewObjectEntity(Level level, int x, int y) {
        return new EtchingStationObjectEntity(level, x, y);
    }
}