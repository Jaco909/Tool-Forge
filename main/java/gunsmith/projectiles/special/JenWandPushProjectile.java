...
public class JenWandPushProjectile extends Projectile {
    float friction;
    public JenWandPushProjectile(){
    }

    public JenWandPushProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    public void doHitLogic(Mob mob, LevelObjectHit object, float x, float y) {
        if (this.isServer()) {
            if (mob != null) {
                ActiveBuff ab = new ActiveBuff(BuffRegistry.getBuff("JenPushedDebuff"), mob, 2F, this.getOwner());
                mob.addBuff(ab, true);
            }
        }
